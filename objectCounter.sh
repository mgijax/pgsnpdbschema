...
#            the ones defined in the pgsnpdbschema product to make sure
...
psql -h ${PG_DBSERVER} -d ${PG_DBNAME} -U ${PG_DBUSER} -t --command "select conname from pg_constraint c join pg_namespace n on n.oid = c.connamespace where c.contype in ('f') and n.nspname = 'snp'" | sed "s/ //g" | grep -v "^$" | sort > ${TMP_DB}
